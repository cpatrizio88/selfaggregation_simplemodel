...
eps_BLs = np.linspace(0.5, 0.6, 5)
eps_as = np.linspace(0.5, 0.6, 5)
...
plevs = np.linspace(0, p_s, 1000)
q_sats = np.zeros(plevs.shape)
for i,p in enumerate(plevs):
    q_sats[i] = wsat(findT(T_s, p), p)
    
def findLCL(qv):
    LCLi = np.argmin(np.abs(qv - q_sat)) 
    p_LCL = plevs[LCLi]
    T_LCL = findT(T_s, p_LCL)
    return (p_LCL, T_LCL) 
...
def equations(p):
    ...
    p_LCL, T_LCL = findLCL(RH_m*q_sat)
