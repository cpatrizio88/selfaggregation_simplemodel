...
domsizes = np.linspace(7500, 200e3, 20)
#colors = ['k', 'r', 'g']



dheatrates = np.array([-1, -1.5])
end = len(dheatrates)
        

for v, dheatrate in enumerate(dheatrates):
    ...
    for j, domsize in enumerate(domsizes):
        ...
        #dheatrate=-2 #prescribed heating rate in moist region interior K/day
        ...
        l_m, f = fsolve(equations, [100e3, 0.95], maxfev=1000000, xtol=1e-16)
    ...
    plt.figure(1)
    plt.plot(l_ds/1e3, massbalance, 'o-', label=r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'mass down - mass up (Pa m$^{2}$ s$^{-1}$)')
    plt.title(r'$Q_{{m,trop}}$ = {:2.1f} K/day, $\alpha$ = {:2.2f}'.format(mheatrate, alpha))
    if v == end-1:
        plt.legend()
        plt.savefig(fout + 'massbalance_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
        plt.close()
    else:
        plt.savefig(fout + 'massbalance_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
    ...
    plt.figure(2)
    plt.plot(l_ds/1e3, waterbalance, 'o-', label=r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'E - P (mm/day)')
    plt.title(r'$Q_{{m,trop}}$ = {:2.1f} K/day, $\alpha$ = {:2.2f}'.format(mheatrate, alpha))
    if v == end-1:
        plt.legend()
        plt.savefig(fout + 'waterbalance_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
        plt.close()
    else:
        plt.savefig(fout + 'waterbalance_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
    ...
    plt.figure(3)
    plt.plot(l_ds/1e3, Ebalance, 'o-', label=r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'condensation warming - radiative cooling (J s$^{-1}$)')
    plt.title(r'$Q_{{m,trop}}$ = {:2.1f} K/day, $\alpha$ = {:2.2f}'.format(mheatrate, alpha))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'Ebalance_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
        plt.close()
    else:
        plt.savefig(fout + 'Ebalance_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
    ...
    plt.figure(4)
    plt.plot(l_ds/1e3, l_ms/1e3, 'o-', label=r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'l$_m$ (km)')
    plt.title(r'$Q_{{m,trop}}$ = {:2.1f} K/day, $\alpha$ = {:2.2f}'.format(mheatrate, alpha))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'lm_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
        plt.close()
    else:
        plt.savefig(fout + 'lm_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
    ...
    plt.figure(5)
    plt.plot(l_ds/1e3, (l_ms**2)/(l_ds**2), 'o-', label=r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'$\sigma$')
    plt.title(r'$Q_{{m,trop}}$ = {:2.1f} K/day, $\alpha$ = {:2.2f}'.format(mheatrate, alpha))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'sigma_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
        plt.close()
    else:
        plt.savefig(fout + 'sigma_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
    ...
    plt.figure(6)
    plt.plot(l_ds/1e3, w_ms, 'o-', label=r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'w$_m$ (m/s)')
    plt.title(r'$Q_{{m,trop}}$ = {:2.1f} K/day, $\alpha$ = {:2.2f}'.format(mheatrate, alpha))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'wm_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
        plt.close()
    else:
        plt.savefig(fout + 'wm_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))

    plt.figure(7)
    plt.plot(l_ds/1e3, RHs, 'o-', label=r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    #plt.ylim(0,1)
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'RH$_m$')
    plt.title(r'$Q_{{m,trop}}$ = {:2.1f} K/day, $\alpha$ = {:2.2f}'.format(mheatrate, alpha))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'RHm_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
        plt.close()
    else:
        plt.savefig(fout + 'RHm_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))

    plt.figure(8)
    plt.plot(l_ds/1e3, Ps, 'o-', label=r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'P (mm/day)')
    plt.title(r'$Q_{{m,trop}}$ = {:2.1f} K/day, $\alpha$ = {:2.2f}'.format(mheatrate, alpha))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'P_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
        plt.close()
    else:
        plt.savefig(fout + 'P_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
    ...
    plt.figure(9)
    plt.plot(l_ds/1e3, delhs, 'o-', label=r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'$\Delta h_{LCL}$ (J)')
    plt.title(r'$Q_{{m,trop}}$ = {:2.1f} K/day, $\alpha$ = {:2.2f}'.format(mheatrate, alpha))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'deltah_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
        plt.close()
    else:
        plt.savefig(fout + 'deltah_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
    ...
    plt.figure(10)
    plt.plot(l_ds/1e3, p_LCLs/1e2, 'o-', label=r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'$p_{LCL}$ (hPa)')
    plt.title(r'$Q_{{m,trop}}$ = {:2.1f} K/day, $\alpha$ = {:2.2f}'.format(mheatrate, alpha))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'pLCL_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
        plt.close()
    else:
        plt.savefig(fout + 'pLCL_varyld_varyQd_alpha{:2.2f}.pdf'.format(alpha))
