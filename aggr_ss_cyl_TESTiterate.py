...
for i, domsize in enumerate(domsizes):
    ...
    #def equations(p):
    #    l_m, f = p
    #    return (l_m - ((omega_BL*l_d**2)/(omega_BL - (S - sig*T_c**4)/(dels_trop + L_v*(f*q_sat -  q_FA))))**(0.5), \
    ...
    l_ms = np.linspace(0, 0.99*l_d, 1000)
    
    massbalance = np.zeros(l_ms.shape)
    waterbalance = np.zeros(l_ms.shape)
    Ebalance = np.zeros(l_ms.shape)
    fs = np.zeros(l_ms.shape)
    delh = np.zeros(l_ms.shape)
    w_ms = np.zeros(l_ms.shape)
    
    
    
    for j, l_m in enumerate(l_ms):
        
        RH_func = lambda RH: 1 + (2*zhat*(q_FAd - q_sat)/(q_sat))/(l_d**2 - l_m**2)*(l_m + zhat - (zhat + l_d)*np.exp((l_m - l_d)/zhat))
        ...
        fs[j] = f
        delh[j] = dels_trop + L_v*(f*q_sat - q_FA)
        w_ms[j] = w_m
        ...
        #
        #omega_mP = (S - sig*T_c**4 + (L_v*P/(np.pi*l_m**2)))/(dels_trop)
        #
        ...
        #print 'w_m = {0} m s^-1'.format(w_m*M_trop)
    ...
    plt.figure(1)
    plt.plot(l_ms/1e3, massbalance, color=col, label=r'l$_d$ = {:5.0f} km'.format(l_d/1e3))
    plt.axhline(0, alpha=0.5)
    plt.xlabel(r'l$_m$ (km)')
    plt.ylabel(r'mass down - mass up (Pa m$^{2}$ s$^{-1}$)')
    plt.ylim(-1e5, 1e5)
    plt.legend()
    plt.savefig(fout + 'massbalance_largeld_varylmld.pdf')

    plt.figure(2)
    plt.plot(l_ms/1e3, waterbalance, color=col, label=r'l$_d$ = {:5.0f} km'.format(l_d/1e3))
    plt.axhline(0, alpha=0.5)
    plt.xlabel(r'l$_m$ (km)')
    plt.ylabel(r'E - P (kg/s)')
    plt.ylim(-1e5, 1e5)
    plt.legend()
    plt.savefig(fout + 'waterbalance_largeld_varylmld.pdf')

    plt.figure(3)
    plt.plot(l_ms/1e3, Ebalance, color=col, label=r'l$_d$ = {:5.0f} km'.format(l_d/1e3))
    plt.axhline(0, alpha=0.5)
    plt.xlabel(r'l$_m$ (km)')
    plt.ylabel(r'condensation warming - radiative cooling (J s$^{-1}$)')
    plt.ylim(-1e5, 1e5)
    plt.legend()
    plt.savefig(fout + 'energybalance_largeld_varylmld.pdf')

    plt.figure(4)
    plt.plot(l_ms/1e3, delh, color=col, label=r'l$_d$ = {:5.0f} km'.format(l_d/1e3))
    plt.axhline(0)
    plt.xlabel(r'l$_m$ (km)')
    plt.ylabel(r'$\Delta h_{trop,conv}$ (J)')
    plt.legend()
    plt.savefig(fout + 'delh_largeld_varylmld.pdf')

    plt.figure(5)
    plt.plot(l_ms/1e3, fs, color=col, label=r'l$_d$ = {:5.0f} km'.format(l_d/1e3))
    plt.axhline(0, alpha=0.5)
    plt.xlabel(r'l$_m$ (km)')
    plt.ylabel('moist region BL relative humidity')
    plt.legend()
    plt.savefig(fout + 'RH_largeld_varylmld.pdf')

    plt.figure(6)
    plt.plot(l_ms/1e3, (l_ms**2)/(l_d**2), color=col, label=r'l$_d$ = {:5.0f} km'.format(l_d/1e3))
    plt.xlabel(r'l$_m$ (km)')
    plt.legend()
    plt.ylabel('convective fractional area')

    plt.savefig(fout + 'convfrac_largeld_varylmld.pdf')

    plt.figure(7)
    plt.plot(l_ms/1e3, w_ms, color=col, label=r'l$_d$ = {:5.0f} km'.format(l_d/1e3))
    plt.xlabel(r'l$_m$ (km)')
    plt.ylabel(r'w$_m$ (m/s)')
    plt.ylim(-10, 10)
    plt.legend()
    plt.savefig(fout + 'wm_largeld_varylmld.pdf')
