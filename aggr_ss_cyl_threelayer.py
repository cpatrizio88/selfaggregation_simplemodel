...
#eps_a = np.linspace(0, 1, 50)

#omega_dFA = np.zeros((eps_mids.size, eps_a.size))
#omega_dBL = np.zeros((eps_mids.size, eps_a.size))
#eps_BL = np.zeros((eps_mids.size, eps_a.size))
...
eps_BL = fsolve(BLcontinuity, 0.5) 

omega_dFA = g*((1-eps_BL)*(1-eps_mid)*sig*T_s**4 + (1-eps_mid)*sig*T_BL**4 + eps_mid*sig*T_mid**4 - 2*eps_a*sig*T_a**4)/(dels_FA)
omega_dBL = g*(eps_mid*eps_a*sig*T_a**4 + (1-eps_BL)*eps_mid*sig*T_s**4 + eps_BL*eps_mid*sig*T_BL**4 - 2*eps_mid*sig*T_mid**4)/dels_mid

w_dFA = omega_dFA/(-rho_midtop*g)
w_dBL = omega_dBL/(-rho_BLtop*g)
