...
domsizes = np.linspace(2000, 40000, 60)
#colors = ['k', 'r', 'g']

#domsizes = np.linspace(200, 6000, 60)


alphas = np.array([0.1, 0.08, 0.05])
#alphas = np.array([0])
...
mheatrates = [0.3, 0.6, 1.0, 1.5]

mheatrates = [-0.2, -0.3]

#mheatrates = [-1.5, -1.0, -0.6, -0.3]
...
for v, mheatrate in enumerate(mheatrates):
    ...
    cplot =  str(v/(1.0*len(mheatrates)))
    ...
    for j, domsize in enumerate(domsizes):
        ...
        #Q_mtrop = (S - sig*T_c**4)
        #mheatrate=0.3#prescribed heating rate in moist region interior K/day
    ...
    plt.figure(1)
    plt.plot(l_ds/1e3, massbalance, '.-', color=cplot, label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'mass down - mass up (Pa m$^{2}$ s$^{-1}$)')
    plt.title(r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    if v == end-1:
        plt.legend()
        plt.savefig(fout + 'massbalance_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'massbalance_varyld_varyQm.pdf')
    ...
    plt.figure(2)
    plt.plot(l_ds/1e3, waterbalance, '.-', color=cplot, label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'E - P (mm/day)')
    plt.title(r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    if v == end-1:
        plt.legend()
        plt.savefig(fout + 'waterbalance_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'waterbalance_varyld_varyQm.pdf')
    ...
    plt.figure(3)
    plt.plot(l_ds/1e3, Ebalance, '.-', color=cplot,label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'condensation warming - radiative cooling (J s$^{-1}$)')
    plt.title(r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'Ebalance_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'Ebalance_varyld_varyQm.pdf')
    ...
    plt.figure(4)
    plt.plot(l_ds/1e3, l_ms/1e3, '.-', color=cplot, label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'l$_c$ (km)')
    plt.title(r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'lm_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'lm_varyld_varyQm.pdf')
    ...
    plt.figure(5)
    plt.plot(l_ds/1e3, (l_ms**2)/(l_ds**2), '.-', color=cplot, label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'$\sigma$')
    plt.title(r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'sigma_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'sigma_varyld_varyQm.pdf')
    ...
    plt.figure(6)
    plt.plot(l_ds/1e3, w_ms, '.-', color=cplot, label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'w$_c$ (m/s)')
    plt.title(r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'wm_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'wm_varyld_varyQm.pdf')

    plt.figure(7)
    plt.plot(l_ds/1e3, RHs, '.-', color=cplot, label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    if mheatrate < 0:
        plt.ylim(0.7,1)
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'RH$_c$')
    ...
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'RHm_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'RHm_varyld_varyQm.pdf')

    plt.figure(8)
    plt.plot(l_ds/1e3, Ps, '.-', color=cplot, label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel(r'P (mm/day)')
    plt.title(r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'P_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'P_varyld_varyQm.pdf')
    ...
    plt.figure(9)
    plt.plot(l_ds/1e3, delhs, '.-', color=cplot, label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel('$\Delta h_{trop}$ (J)')
    plt.title(r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'deltah_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'deltah_varyld_varyQm.pdf')
    ...
    plt.figure(10)
    plt.plot(l_ds/1e3, delhseff, '.-', color=cplot, label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel('$\Delta h_{trop,eff}$ (J)')
    plt.title(r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'deltaheff_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'deltaheff_varyld_varyQm.pdf')
    ...
    plt.figure(11)
    plt.plot(l_ds/1e3, (l_ds - l_ms)/1e3, '.-', color=cplot, label=r'$Q_{{c,trop}}$ = {:2.1f} K/day'.format(mheatrate))
    plt.xlabel(r'l$_d$ (km)')
    plt.ylabel('$l_d - l_c$ (J)')
    plt.title(r'$Q_{{d,trop}}$ = {:2.1f} K/day'.format(dheatrate))
    if v == end-1:
        plt.legend(loc='best')
        plt.savefig(fout + 'ldlc_varyld_varyQm.pdf')
        plt.close()
    else:
        plt.savefig(fout + 'ldlc_varyld_varyQm.pdf')
...
#plt.figure(10)
#plt.plot(l_ds/1e3, w_BLpluss,'.-', label=r'$w_{BL+}$')
#plt.plot(l_ds/1e3, w_BLminuss, '.-', label=r'$w_{BL-}$')
#plt.xlabel(r'l$_d$ (km)')
#plt.ylabel(r'$w$ (m/s)')
#plt.legend()
