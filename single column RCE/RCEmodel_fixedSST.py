...
To = 250 #initial temperature of atmosphere (K)
N = 40 #number layers
...
Tinit[0] = T_sinit
#Tinit[0] = T_sinit
#Tinit = np.loadtxt("T0profile_fixedSST")
#T_sinit = Tinit[0]
...
qv = np.zeros(N+1)
BL_index = np.where(p_BL >= plevs)[0][0]
trop_index = np.where(p_trop >= plevs)[0][0]
qv[plevs >= p_BL] = q_BLbar 
qv[plevs < p_BL] = q_FA
qv[plevs < p_trop] = q_strat
3
...
epssw = np.ones(N+1)*0.035
for i in range(N):
 epssw[N-i-1] = 0.5*epssw[N-i]
epssw[plevs > p_trop] = 0
...
#while np.any(Trdiff > eps):
    ...
    #compute emissivity at every step.
    ...
    print 'after convective adjustment', T

np.savetxt("T0profile_fixedSST", T)
T_s = T[0]

#effective temperature of free troposphere (i.e. brightness temperature at p_trop)
T_a = (LWupt[trop_index]/sig)**(0.25)
...
#Tinit = np.loadtxt('T0profilenew2')
...
plt.plot(T, plevs/100., 'x-', label='RCE profile, N = {0} layers, q_strat = {1}, q_FA = {2}, q_BLbar = {3}, '.format(N, q_strat, q_FA, q_BLbar))
plt.plot(T_pierrehumb, plevs/100., 'x-', label='Pierre humbert profile, zeta_T = {0} km, gamma = {1} K km^-1'.format(zeta_T, gamma_PH))
...
plt.title('radiative-convective equilibrium temperature profile w/ fixed SST = {0} K'.format(T[0]) )
plt.legend()
plt.show()

plt.figure(6)
plt.plot(T_pierrehumb, plevs/100., color='r', label='Pierre humbert profile, zeta_T = {0} km, gamma = {1} K km^-1'.format(zeta_T, gamma_PH))
...
plt.figure(7)
plt.plot(epslw, plevs/100., 'x-', label='eps_lw, f={0} GHz'.format(f))
...
plt.figure(8)
plt.plot(epssw, plevs/100., 'x-', label='eps_sw')
plt.ylabel('pressure (hPa)')
plt.xlabel('emissivity')
plt.gcf().gca().invert_yaxis()
plt.legend()
