...
# This code simply asks for user to input the frequency,
# temperature, pressure, relative humidity and cloud liquid
# water and calls the absorb subroutine to get the atmospheric
# extinction (really absorbtion since there is no scattering).
#
# usage:      absorb

#FREQY,TEMP,PRES,W = np.asarray(input("Enter Freq [GHz], T [K], Pres [mb], h2omix [kg/kg]: "),dtype=np.float32)

def ABSORB_L93(FREQY,TEMP,PRES,W,CWC):
#     Convert retrieval quantities to parameters needed by Liebe93
#
#
#      FREQY : frequency in GHz
#      TEMP : Temperature in Kelvin
#      PRES : total pressure in hPa
#      W    : water vapor mass mixing ratio in kg/kg
#      CWC  : cloud water content in g/m^3

  F  = FREQY              # frequency [GHz]
  Tc = TEMP - 273.15      # temperature [C] 
  Pb = 0.1*PRES           # total pressure [KPa] 
  E  = W * Pb/(0.622 + W) # pressure of water vapor [kPa]
  R  = CWC                # cloud water content [g/m^3]
  ATM_EXT=MPM93(F,Pb,E,Tc,R)
  return ATM_EXT
...
#       COMMON /MWLINES2/ F0O2,A, F0H2O,B
  ...
  IFIRST=0
  ICE=0  #hardcoded by JLH

#     The following data was in mwlines.93.data      
  F0O2 =[      50.474239,   50.987747,   51.503349,   52.021412, \
  ...
  A1  = [       0.094,    0.246,    0.608,    1.414,    3.102,  \
  ...
  A2 = [        9.694,    8.694,    7.744,    6.844,    6.004,  \
  ...
  A3 = [        0.890,    0.910,    0.940,    0.970,    0.990,  \
  ...
  A5 = [        0.240,    0.220,    0.197,    0.166,    0.136,  \
  ...
  A6 = [        0.790,    0.780,    0.774,    0.764,    0.751,  \
		...
		0.000,    0.000,    0.000,    0.000 ]
  F0H2O = [   22.235081,   67.803963,  119.995941,  183.310089,  \
  ...
  B1 = [       0.01130,    0.00012,    0.00008,    0.24200,  \
  ...
  B2 = [     2.143,    8.735,    8.356,    0.668,    6.181,   \
  ...
  B3 = [     2.811,    2.858,    2.948,    3.050,    2.303,   \
  ...
  B4 = [     4.80,    4.93,    4.78,    5.30,    4.69,    4.85,   \
  ...
  B5 = [     0.69,    0.69,    0.70,    0.64,    0.67,    0.68,   \
  ...
  B6 = [     1.00,    0.82,    0.79,    0.85,    0.54,    0.74,   \
  ...
  for I in range(0,44):
    GAMMA=0.0
    S=A[0,I]*P*V**3*m.exp(A[1,I]*(1.-V))*1.E-6
    GAMMA=A[2,I]*(P*V**(0.8-A[3,I])+1.1*E*V)*1.E-3
    GAMMA=(GAMMA**2+(25*0.6E-4)**2)**0.5
    DELTA=(A[4,I]+A[5,I]*V)*(P+E)*(V**0.8)*1.E-3
    ZF=F/F0O2[I]*(complex(1.,-DELTA)/complex(F0O2[I]-F,-GAMMA)-complex(1.,DELTA)/complex(F0O2[I]+F,GAMMA))
    ZN=ZN+S*ZF
  ...
  ZFo=-F/complex(F,GAMMAo)
  Sn=1.40E-12*P**2*V**3.5
  ZFn=complex(0.,F/(1.93E-5*F**1.5+1.))
  ZN=So*ZFo+Sn*ZFn
  ...
  for I in range(0,35):
    GAMH=0.
    S=B[0,I]*E*V**3.5*m.exp(B[1,I]*(1.-V))
  # Doppler approximation.
    GAMH=B[2,I]*(P*V**B[4,I]+B[3,I]*E*V**B[5,I])*1.E-3
    GAMD2=1E-12/V*(1.46*F0H2O[I])**2
    GAMH=0.535*GAMH+(0.217*GAMH**2+GAMD2)**0.5
    DELH=0.
    ZF=F/F0H2O[I]*(complex(1.,-DELH)/complex(F0H2O[I]-F,-GAMH)-complex(1.,DELH)/complex(F0H2O[I]+F,GAMH))
    ZN=ZN+S*ZF
  ...
  #JLH    fD=20.20-146.4*(V-1)+316*(V-1)^2
    fD=20.1*m.exp(7.88*(1-V))
    fS=39.8*fD
    Eps=103.3*(V-1)+77.66
    Epinf=0.0671*Eps
    Eopt=3.52
  # Complex Permittivity of water (double-Debye model)
    ZEp=Eps-F*((Eps-Epinf)/complex(F,fD)+(Epinf-Eopt)/complex(F,fS))
  #
  # ICE PERMITTIVITY [8]
  else:
    Ai=(62.*V-11.6)*1.E-4*m.exp(-22.1*(V-1.))
    Bi=.542E-6*(-24.17+116.79/V+(V/(V-.9927))**2)
    Eps=3.15
  # Complex Permittivity of Ice 
    fice=F
    if F<.001: fice=.001
    ZEp=complex(3.15,Ai/fice+Bi*fice)
...
#atm_ext=ABSORB_L93(FREQY,TEMP,PRES,W,0.)
#print ' Atm_ext [km^-1] = ',atm_ext
