...
for i, eps_BL in enumerate(eps_BLs):
    ...
    for j, eps_a in enumerate(eps_as):
        ...
        def equations_new(p):
            ...
            Q_dtrop = sig*eps_a*(eps_strat*T_strat**4 + eps_BL*T_BL**4 + (1 - eps_BL)*T_s**4 - 2*T_a**4)
        ...
        p_BL, = fsolve(pBL_fn, [950e2], maxfev=100000)     
...
#        #T_BLs[i,j] = T_BL
#        
#        omega_BLs[i,j] = g*eps_a*(eps_strat*sig*T_strat**4 + eps_BL*sig*T_BLfn(p_BL)**4 + (1-eps_BL)*sig*T_s**4 - 2*sig*T_afn(p_BL)**4)/dels_tropfn(p_BL) 
...
#        rho_BL = (rho_BLtop + rho_s)/2.
#        
#        w_BLs[i,j] = omega_BLs[i,j]/(-rho_BL*g) 
#        
#        
#        T_a = T_afn(p_BL)
#        T_BL = T_BLfn(p_BL) 
...
#        
#plt.figure(1)
#plt.contourf(eps_BLss, eps_ass, p_BLs/1e2, cpBL, cmap=cm.RdYlBu_r)
#plt.xlabel(r'$\epsilon_{BL}$')
#plt.ylabel(r'$\epsilon_{a}$')
#plt.colorbar(label=r'$p_{BL}$ (hPa)')
#plt.show()
#
#plt.figure(2)
#plt.contourf(eps_BLss, eps_ass, w_BLs, 100, cmap=cm.RdYlBu_r)
#plt.xlabel(r'$\epsilon_{BL}$')
#plt.ylabel(r'$\epsilon_{a}$')
#plt.colorbar(label=r'$w_{BL}$ (m/s)')
#plt.show()
...
#
#plt.figure(3)
#plt.semilogy(findT(T_s, plevs), plevs/1e2)
