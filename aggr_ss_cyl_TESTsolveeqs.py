...
from scipy.optimize import fsolve, brentq, newton_krylov
...
eps_BLs = np.linspace(0.7, 1, 1)
eps_as = np.linspace(0.1, 1, 1)
...
plevs = np.linspace(500e2, p_s, 1000)    
...
def T_afn(p_BL, p_out):
    #T_a = np.mean(findTmoist(thetae0, np.linspace(p_BL, p_out)))
    T_a = np.mean(findT(T_s, np.linspace(p_BL, p_out)))
    return T_a
    
def dels_tropfn(p_BL, p_out):
    #T_BLtop = findTmoist(thetae0, p_BL) #temperature of boundary layer top 
    T_BLtop = findT(T_s, p_BL)
...
def equations(p):
    l_m, RH_m, p_out, p_BL = p
    #thickness = lambda p: (c.Rd/(p*g))*findTmoist(thetae0, p)   
    thickness = lambda p: (c.Rd/(p*g))*findT(thetae0, p)   
    #T_BLtop = findTmoist(thetae0, p_BL) #temperature of boundary layer top
    ...
    dels_trop = dels_tropfn(p_BL, p_out)
    
    #T_strat = findTmoist(thetae0, p_out)
    T_strat = findT(T_s, p_out)
    T_a = T_afn(p_BL, p_out)
    T_BL = T_BLfn(p_BL)
    
    z_out = integrate.quad(thickness, p_out, p_s)[0]
    z_t = integrate.quad(thickness, p_t, p_s)[0]

    
    q_FA = wsat(T_strat, p_out)
    ...
    #p_LCL, T_LCL = findLCL0(RH_m*q_sat, p_s, T_BL)
    #T_LCL = findTmoist(thetae0, p_LCL)
    p_LCL = p_BL
    T_LCL = T_BLtop
    delz_LCL = integrate.quad(thickness, p_out, p_LCL)[0]
    dels_LCL = c.cpd*(T_LCL-T_strat) + g*(-delz_LCL)
    
    omega_m = (g*Q_mtrop)/(dels_LCL + L_v*(RH_m*q_sat -  q_FA))
    
    
    f1 = (omega_BL - omega_m)*l_m**2 - (omega_BL*l_d**2)
    f2 = (RH_m-1)*q_sat - (2*zhat*(q_FA - q_sat)/(l_d**2 - l_m**2))*(l_m + zhat - (zhat + l_d)*np.exp((l_m - l_d)/zhat))
    f3 = Q_dtrop/dels_tropfn(p_BL, p_out) - Q_dBL/dels_BLfn(p_BL)
            
    f4 = c.cpd*T_strat + g*(z_out + z_t)/2. + L_v*q_FA - (c.cpd*T_BL + (g*delz_BL)/2. + L_v*RH_m*q_sat)
...
for i, eps_BL in enumerate(eps_BLs):
    ...
    for j, eps_a in enumerate(eps_as):
        ...
        #pout_fn = lambda p_out: eps_a*(eps_strat*sig*np.power(T_stratfn(p_out), 4) + eps_BL*sig*T_BLfn(p_BL)**4 + (1-eps_BL)*sig*T_s**4 - 2*sig*T_afn(p_BL)**4)/dels_tropfn(p_out) - eps_BL*((1-eps_a)*eps_strat*sig*np.power(T_stratfn(p_out), 4) + eps_a*sig*T_afn(p_out)**4 + sig*T_s**4 - 2*sig*T_BLfn(p_BL)**4)/dels_BLfn(p_BL)
        ...
        #p_out = fsolve(pout_fn, 200e2)[0]
 
        init = [500e3, 0.5, 200e2, 950e2]
        print equations(init)
        l_m, RH_m, p_out, p_BL = fsolve(equations, init)
        #l_m, RH_m, p_out, p_BL = newton_krylov(equations, init)
        
        T_out = T_stratfn(p_out)
        
        p_outs[i,j] = p_out
        
        #p_BLs[i,j] = p_BL
        
        #T_a = fsolve(Ta_fn, 250)[0]
        #T_as[i,j] = T_a
...
plt.figure(1)
plt.contourf(eps_BLss, eps_ass, p_outs/1e2, 100, cmap=cm.RdYlBu_r)
plt.xlabel(r'$\epsilon_{BL}$')
plt.ylabel(r'$\epsilon_{a}$')
plt.colorbar(label=r'$p_{out}$ (hPa)')
plt.show()
