...
domsizes = np.linspace(3000, 8000, 20)
#colors = ['k', 'r', 'g']
...
for j, domsize in enumerate(domsizes):
    ...
    def equations(p):
        ...
        return (l_m - ((omega_BL*l_d**2)/(omega_BL - g*(S - sig*T_c**4)/(dels_LCL + L_v*(f*q_sat -  q_FA))))**(0.5), \
               (f-1)*q_sat - (2*zhat*(q_FAd - q_sat))/(l_d**2 - l_m**2)*(l_m + zhat - (zhat + l_d)*np.exp((l_m - l_d)/zhat)))
    

    l_m, f = fsolve(equations, [100e3, 0.5], maxfev=100000, xtol=1e-13)
    ...
    BLcontinuity[j] = w_BLplus - w_BLminus
...
plt.figure(10)
plt.plot(l_ds/1e3, BLcontinuity, 'o-')
plt.xlabel(r'l$_d$ (km)')
plt.ylabel(r'$w_{BL+} - w_{BL-}$ (m/s)')
