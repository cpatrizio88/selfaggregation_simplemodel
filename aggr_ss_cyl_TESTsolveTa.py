...
eps_BLs = np.linspace(0.8, 1, 30)
eps_as = np.linspace(0.5, 0.6, 30)
#eps_strats = np.linspace(0, 1, 30)


eps_BLss, eps_ass = np.meshgrid(eps_BLs, eps_as)
#eps_stratss, eps_ass = np.meshgrid(eps_strats, eps_BLs)

#p_BLs = np.zeros(eps_BLss.shape)
T_as = np.zeros(eps_BLss.shape)

#T_as = np.zeros(eps_stratss.shape)
...
for i, eps_BL in enumerate(eps_BLs):
    ...
    #for j, eps_strat in enumerate(eps_strats):
        ...
        pBL_fn = lambda p_BL: eps_a*(eps_strat*sig*T_strat**4 + eps_BL*sig*T_BL**4 + (1-eps_BL)*sig*T_s**4 - 2*sig*T_a**4)/dels_tropfn(p_BL) - eps_BL*((1-eps_a)*eps_strat*sig*T_strat**4 + eps_a*sig*T_a**4 + sig*T_s**4 - 2*sig*T_BL**4)/dels_BLfn(p_BL)
...
plt.figure(1)
plt.contourf(eps_BLss, eps_ass, T_as, 100, cmap=cm.RdYlBu_r)
plt.xlabel(r'$\epsilon_{BL}$')
plt.ylabel(r'$\epsilon_{a}$')
plt.colorbar(label=r'$T_a$ (K)')
plt.show()

#plt.figure(1)
#plt.contourf(eps_BLss, eps_stratss, T_as, 100, cmap=cm.RdYlBu_r)
#plt.xlabel(r'$\epsilon_{BL}$')
#plt.ylabel(r'$\epsilon_{strat}$')
#plt.colorbar(label=r'$T_a$ (K)')
#plt.show()
